...
class _ComparisonResultScreenState extends State<ComparisonResultScreen> {
  ...
  Future<void> _shareResults() async {
    try {
      ...
      await ShareService.captureAndShare(
        ...
        fileName: 'comparison_result_${DateTime.now().millisecondsSinceEpoch}',
        shareText: 'IngredientLens - ${AppLocalizations.of(context)!.translate('compare_ingredients')}',
      );
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
  ...
  List<Widget> _buildSectionsWithSpacing(BuildContext context) {
    ...
    // 광고 섹션 (종합 비교 분석 후에) - 캡처 모드가 아닐 때만 표시
